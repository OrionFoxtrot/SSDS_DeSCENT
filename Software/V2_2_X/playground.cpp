...
public:
  ...
  static void serialize(const DataPacket &data, uint8_t *buffer)
  {
    ...
    // GPS (14 bytes)
    write_i32(buffer, pos, data.gpsData.latitude);
    pos += 4;
    write_i32(buffer, pos, data.gpsData.longitude);
    pos += 4;
    write_i32(buffer, pos, data.gpsData.altitude);
    pos += 4;
    write_u16(buffer, pos, data.gpsData.time);
    pos += 2;

    // IMU (18 bytes)
    write_i16(buffer, pos, data.imuData.gyroX);
    pos += 2;
    write_i16(buffer, pos, data.imuData.gyroY);
    pos += 2;
    write_i16(buffer, pos, data.imuData.gyroZ);
    pos += 2;
    write_i16(buffer, pos, data.imuData.linX);
    pos += 2;
    write_i16(buffer, pos, data.imuData.linY);
    pos += 2;
    write_i16(buffer, pos, data.imuData.linZ);
    pos += 2;
    write_i16(buffer, pos, data.imuData.magX);
    pos += 2;
    write_i16(buffer, pos, data.imuData.magY);
    pos += 2;
    write_i16(buffer, pos, data.imuData.magZ);
    pos += 2;

    // BME (6 bytes)
    write_i16(buffer, pos, data.bmeData.temp);
    pos += 2;
    write_i16(buffer, pos, data.bmeData.pressure);
    pos += 2;
    write_i16(buffer, pos, data.bmeData.humidity);
    pos += 2;
  ...
  static bool deserialize(const uint8_t *buffer, size_t len, DataPacket &data)
  {
    ...
    // GPS
    data.gpsData.latitude = read_i32(buffer, pos);
    pos += 4;
    data.gpsData.longitude = read_i32(buffer, pos);
    pos += 4;
    data.gpsData.altitude = read_i32(buffer, pos);
    pos += 4;
    data.gpsData.time = read_u16(buffer, pos);
    pos += 2;

    // IMU
    data.imuData.gyroX = read_i16(buffer, pos);
    pos += 2;
    data.imuData.gyroY = read_i16(buffer, pos);
    pos += 2;
    data.imuData.gyroZ = read_i16(buffer, pos);
    pos += 2;
    data.imuData.linX = read_i16(buffer, pos);
    pos += 2;
    data.imuData.linY = read_i16(buffer, pos);
    pos += 2;
    data.imuData.linZ = read_i16(buffer, pos);
    pos += 2;
    data.imuData.magX = read_i16(buffer, pos);
    pos += 2;
    data.imuData.magY = read_i16(buffer, pos);
    pos += 2;
    data.imuData.magZ = read_i16(buffer, pos);
    pos += 2;

    // BME
    data.bmeData.temp = read_i16(buffer, pos);
    pos += 2;
    data.bmeData.pressure = read_i16(buffer, pos);
    pos += 2;
    data.bmeData.humidity = read_i16(buffer, pos);
    pos += 2;
