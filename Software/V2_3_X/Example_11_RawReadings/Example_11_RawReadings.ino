...
#define BNO08X_INT  PB3
//#define BNO08X_INT  -1
#define BNO08X_RST  PB4
//#define BNO08X_RST  -1
...
void setup() {
  ...
  while(!Print_tx_rx) delay(10); // Wait for Serial to become available.
...
void setReports(void) {
  ...
  if (myIMU.enableRawAccelerometer(1) == true) {
    Print_tx_rx.println(F("Raw Accelerometer enabled"));
  } else {
    Print_tx_rx.println("Could not enable raw accelerometer");
  }

  if (myIMU.enableGyro(1) == true) {
    Print_tx_rx.println(F("Gyro enabled"));
  } else {
    Print_tx_rx.println("Could not enable gyro");
  }

  if (myIMU.enableRawGyro(1) == true) {
    Print_tx_rx.println(F("Raw Gyro enabled"));
  } else {
    Print_tx_rx.println("Could not enable raw gyro");
  }

  if (myIMU.enableMagnetometer(1) == true) {
    Print_tx_rx.println(F("Magnetometer enabled"));
  } else {
    Print_tx_rx.println("Could not enable Magnetometer");
  }

  if (myIMU.enableRawMagnetometer(1) == true) {
    Print_tx_rx.println(F("Raw Magnetometer enabled"));
  } else {
    Print_tx_rx.println("Could not enable Raw Magnetometer");
  }

  Print_tx_rx.println(F("Raw MEMS readings enabled"));
  Print_tx_rx.println(F("Output is: (accel) x y z (gyro) x y z (mag) x y z"));
}
...
void loop() {
  ...
  if (myIMU.getSensorEvent() == true)
  {
    ...
    if(timeSinceLastSerialPrint > DEBUG_INTERVAL_MILLISECONDS)
    {
      ...
      Print_tx_rx.print(gx);
      Print_tx_rx.print("\t");
      Print_tx_rx.print(gy);
      Print_tx_rx.print("\t");
      Print_tx_rx.print(gz);
      Print_tx_rx.print("\t");

      Print_tx_rx.print(mx);
      Print_tx_rx.print("\t");
      Print_tx_rx.print(my);
      Print_tx_rx.print("\t");
      Print_tx_rx.print(mz);
      Print_tx_rx.print("\t");

      Print_tx_rx.print(timeSinceLastSerialPrint);
