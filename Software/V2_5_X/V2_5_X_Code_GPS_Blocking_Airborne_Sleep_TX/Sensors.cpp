...
// u-blox M10 configuration keys. These are defined here explicitly because
// some SparkFun GNSS library releases do not include every M10 key.
constexpr uint32_t kUBXCfgRateMeas = 0x30210001;        // CFG-RATE-MEAS, U2, milliseconds.
constexpr uint32_t kUBXCfgRateNav = 0x30210002;         // CFG-RATE-NAV, U2, measurement cycles.
constexpr uint32_t kUBXCfgPMOperateMode = 0x20D00001;   // CFG-PM-OPERATEMODE, E1.
constexpr uint32_t kUBXCfgNavspgDynmodel = 0x20110021;  // CFG-NAVSPG-DYNMODEL, E1.

constexpr uint8_t kGPSOperateModeFull = 0;                  // Normal full-power operation.
constexpr uint16_t kGPSNormalMeasurementIntervalMs = 1000;  // Continuous 1 Hz navigation.

// UBX-CFG-VALSET layer bit mask: RAM = bit 0, BBR = bit 1. Writing both
// clears any PSM setting retained by earlier firmware and preserves the
// airborne model across receiver backup-domain resets.
constexpr uint8_t kGPSConfigLayers = 0x03;
...
constexpr uint8_t kAllFreshIMUReportsMask =
  kFreshLinearAccelerationBit | kFreshGyroscopeBit | kFreshMagnetometerBit | kFreshOrientationBit;  // 1111
...
bool Sensors::beginGPS() {
  ...
  for (uint8_t attempt = 0; attempt < kInitializationAttempts; attempt++) {
    if (_gps.begin(_gpsSerial)) {
      // Keep only UBX output on UART to reduce unsolicited NMEA traffic.
      const bool uartOkay = _gps.setUART1Output(COM_TYPE_UBX);

      // Force normal continuous operation at 1 Hz. Before the first fix the
      // receiver uses its normal full-power acquisition behavior; afterward it
      // remains in ordinary tracking. This also clears PSM settings retained by
      // any earlier firmware.
      const bool airborneOkay = configureGPSAirborneMode();
...
bool Sensors::configureGPSNormalContinuousMode() {
  // Store normal full-power operation in RAM and BBR. Acquisition and tracking
  // are handled automatically by the receiver without later mode transitions.
  const bool rateOkay = _gps.setVal16(
    kUBXCfgRateMeas,
    kGPSNormalMeasurementIntervalMs,
    kGPSConfigLayers);

  const bool navOkay = _gps.setVal16(
    kUBXCfgRateNav,
    1,
    kGPSConfigLayers);

  const bool modeOkay = _gps.setVal8(
    kUBXCfgPMOperateMode,
    kGPSOperateModeFull,
    kGPSConfigLayers);

  return rateOkay && navOkay && modeOkay;
}

bool Sensors::configureGPSAirborneMode() {
  const bool okay = _gps.setVal8(
    kUBXCfgNavspgDynmodel,
...
bool Sensors::readGPS(uint16_t maxWaitMs) {
  // Do not poll while the receiver is in software standby. Any UART traffic is
  // a wake source, so polling here would unintentionally wake the MAX-M10S.
  ...
  // Airborne dynamic models do not support a 2D solution, so require a valid
  // 3D-or-better fix before accepting the position.
  const uint8_t fixType = _gps.getFixType();
  data.gps.valid = (fixType >= 3) && !_gps.getInvalidLlh();

  if (data.gps.valid) {
    _gpsHasEverLocked = true;
  }
...
bool Sensors::waitForGPSFix(uint32_t statusIntervalMs,
  ...
  const uint32_t startMs = millis();
  uint32_t previousStatusMs = startMs;

  while (true) {
    // Keep draining IMU reports while setup is blocked so its event queue does
    // not fill during a long GNSS acquisition.
    service();
...
bool Sensors::sleepGPS(uint32_t durationMs) {
  ...
  // durationMs == 0 keeps the receiver in software standby until UART RX
  // activity wakes it. The last valid data remains in SensorData for encoding.
  ...
  // Allow the receiver to finish entering standby before the radio current
  // step begins.
  delay(50);
  _gpsSleeping = true;
...
bool Sensors::wakeGPS(uint16_t wakeDelayMs) {
  ...
  // A UART start bit wakes the receiver. 0xFF is not a complete UBX message,
  // so it is discarded after serving as the wake event.
  _gpsSerial.write(static_cast<uint8_t>(0xFF));
  _gpsSerial.flush();
  delay(wakeDelayMs);
  _gpsSleeping = false;

  // Software standby clears normal RAM. Reapply the live UART configuration,
  // airborne dynamic model, and normal 1 Hz full-power acquisition/tracking
  // mode. The airborne and operating-mode settings are also stored in BBR.
  const bool uartOkay = _gps.setUART1Output(COM_TYPE_UBX);
  const bool airborneOkay = configureGPSAirborneMode();
  const bool normalModeOkay = configureGPSNormalContinuousMode();
...
bool Sensors::gpsHasEverLocked() const {
  return _gpsHasEverLocked;
}
