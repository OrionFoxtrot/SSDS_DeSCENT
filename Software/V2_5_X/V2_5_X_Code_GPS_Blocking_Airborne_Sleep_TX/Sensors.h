...
// u-blox dynamic platform model value. Airborne <4g is appropriate for the
// highly dynamic ChipSat flight. Change this to 6 for airborne <1g or 7 for
// airborne <2g if a lower-dynamics model is preferred.
#ifndef CHIPSAT_GPS_DYNAMIC_MODEL
#define CHIPSAT_GPS_DYNAMIC_MODEL 8
#endif
...
public:
  ...
  // The MAX-M10S starts in normal full-power acquisition. setup() can block
  // on waitForGPSFix(). In loop(), do not call readGPS while the receiver is in
  // software standby because UART traffic is also its wake source.
  bool readGPS(uint16_t maxWaitMs = 1200);

  // Blocks indefinitely until a valid 3D GPS fix is obtained. Intended for use
  // from setup() before the high-power radio is initialized or used.
  bool waitForGPSFix(uint32_t statusIntervalMs = 5000,
                     uint16_t pollWaitMs = 1200);
  ...
  bool readSlowSensors(uint16_t gpsMaxWaitMs = 1200);

  // Low-power controls used around the high-current LoRa transmission.
  // sleepGPS(0) enters software standby indefinitely until UART RX activity
  // wakes the MAX-M10S. The airborne model and normal 1 Hz operating mode are
  // written to BBR and are also reapplied after every wake.
  bool sleepIMU();
  bool wakeIMU();
  bool sleepGPS(uint32_t durationMs = 0);
  bool wakeGPS(uint16_t wakeDelayMs = 500);

  // Convenience pair for wrapping a radio transmission. sleepForTransmit()
  // sleeps the GPS first, then the IMU. wakeAfterTransmit() attempts to wake
  // both sensors even if one of the operations fails.
  bool sleepForTransmit();
  bool wakeAfterTransmit(uint16_t gpsWakeDelayMs = 500);

  bool imuSleeping() const;
  bool gpsSleeping() const;
  bool gpsHasEverLocked() const;
...
private:
  ...
  bool beginGPS();
  bool configureGPSNormalContinuousMode();
  bool configureGPSAirborneMode();
